...
public class ExpiryPolicyBuilder2<K, V> implements Builder<ExpiryPolicy<K, V>> {
	...
	public static Duration getExpiry(Object key) {
		return durationMap.get(key);
	}

	/**
	 * Get an {@link ExpiryPolicy} instance for a non expiring (ie. "eternal")
	 * cache.
	 *
	...
	/**
	 * Get a time-to-live (TTL) {@link ExpiryPolicy} instance for the given
	 * {@link Duration}.
	 *
	...
	/**
	 * Get a time-to-idle (TTI) {@link ExpiryPolicy} instance for the given
	 * {@link Duration}.
	 *
	...
	/**
	 * Fluent API for creating an {@link ExpiryPolicy} instance where you can
	 * specify constant values for creation, access and update time. Unspecified
	 * values will be set to {@link ExpiryPolicy#INFINITE INFINITE} for create and
	 * {@code null} for access and update, matching the {@link #noExpiration()} no
	 * expiration} expiry.
	 *
	 * @return an {@link ExpiryPolicy} builder
	 */
	public static ExpiryPolicyBuilder2<Object, Object> expiry() {
		return new ExpiryPolicyBuilder2<>();
	}
	...
	private ExpiryPolicyBuilder2() {
		this((k, v) -> ExpiryPolicy.INFINITE, (k, v) -> null, (k, oldV, newV) -> null);
	}

	private ExpiryPolicyBuilder2(BiFunction<? super K, ? super V, Duration> create,
	 ...
	 * <p>
	 * Note: Calling this method on a builder with an existing TTL since creation
	 * will override the previous value or function.
	 *
	 * @param create TTL since creation
	 * @return a new builder with the TTL since creation
	 */
	public ExpiryPolicyBuilder2<K, V> create(Duration create) {
	 ...
	 */
	public <K2 extends K, V2 extends V> ExpiryPolicyBuilder2<K2, V2> create(BiFunction<K2, V2, Duration> create) {
		return new ExpiryPolicyBuilder2<>(Objects.requireNonNull(create), access, update);
	}

	/**
	 * Set TTI since last access.
	 * <p>
	 * Note: Calling this method on a builder with an existing TTI since last access
	 * will override the previous value or function.
	 *
	 * @param access TTI since last access
	 * @return a new builder with the TTI since last access
	 */
	public ExpiryPolicyBuilder2<K, V> access(Duration access) {
	 ...
	 */
	public <K2 extends K, V2 extends V> ExpiryPolicyBuilder2<K2, V2> access(
			BiFunction<K2, Supplier<? extends V2>, Duration> access) {
		return new ExpiryPolicyBuilder2<>(create, Objects.requireNonNull(access), update);
	}

	/**
	 * Set TTL since last update.
	 * <p>
	 * Note: Calling this method on a builder with an existing TTL since last access
	 * will override the previous value or function.
	 *
	 * @param update TTL since last update
	 * @return a new builder with the TTL since last update
	 */
	public ExpiryPolicyBuilder2<K, V> update(Duration update) {
	 ...
	 * <p>
	 * Note: Calling this method on a builder with an existing TTL since last update
	 * will override the previous value or function.
	 *
	 ...
	 */
	public <K2 extends K, V2 extends V> ExpiryPolicyBuilder2<K2, V2> update(
			TriFunction<K2, Supplier<? extends V2>, V2, Duration> update) {
		return new ExpiryPolicyBuilder2<>(create, access, Objects.requireNonNull(update));
	}
	...
	/**
	 * Simple implementation of the {@link ExpiryPolicy} interface allowing to set
	 * constants to each expiry types.
	 */
	private static class BaseExpiryPolicy<K, V> implements ExpiryPolicy<K, V> {
		...
		public Duration getExpiryForCreation(K key, V value) {
			Duration duration = ExpiryPolicyBuilder2.getExpiry(key);
			if (null != duration) {
				return duration;
			} else {
